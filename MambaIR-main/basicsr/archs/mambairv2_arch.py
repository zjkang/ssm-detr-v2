...
#         x = self.conv_first(x)
#         # 1.2 深层特征提取
#         x = self.forward_features(x, params)
#         # 1.3 图像重建
#         x = self.conv_last(x)

# # 2. 深层特征提取：ASSB (Attentive State Space Block)
# class ASSB(nn.Module):
#     def forward(self, x, x_size, params):
#         # 2.1 图像分块
...
# # 4. 注意力层：AttentiveLayer
# class AttentiveLayer(nn.Module):
#     def forward(self, x, x_size, params):
#         # 4.1 窗口注意力
#         x_win = self.win_mhsa(x, params)
#         # 4.2 状态空间处理
#         x_aca = self.assm(x, x_size, self.embeddingA)
...
#         └── BasicBlock
#             └── AttentiveLayer
#                 ├── WindowAttention (win_mhsa)
#                 └── ASSM
#                     ├── 特征嵌入 (embeddingB)
#                     ├── 路由预测 (route)
#                     ├── 特征重组 (semantic_neighbor)
#                     └── Selective_Scan
#                         └── forward_core
#                             ├── 特征投影 (einsum)
#                             ├── 状态空间处理
#                             └── 选择性扫描 (selective_scan)
