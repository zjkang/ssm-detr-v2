...
class HungarianMatcher(nn.Module):
    ...
    def calculate_cost(self, pred_boxes: Tensor, pred_logits: Tensor, gt_boxes: Tensor, gt_labels: Tensor):
        ...
        if torch.isnan(cost_class).any() or torch.isinf(cost_class).any():
            print("Warning: Invalid values in cost_class")
            cost_class = torch.nan_to_num(cost_class, nan=0.0, posinf=1e6, neginf=-1e6)
        
        if torch.isnan(cost_bbox).any() or torch.isinf(cost_bbox).any():
            print("Warning: Invalid values in cost_bbox")
            cost_bbox = torch.nan_to_num(cost_bbox, nan=0.0, posinf=1e6, neginf=-1e6)
        
        if torch.isnan(cost_giou).any() or torch.isinf(cost_giou).any():
            print("Warning: Invalid values in cost_giou")
            cost_giou = torch.nan_to_num(cost_giou, nan=0.0, posinf=1e6, neginf=-1e6)
        ...
        if torch.isnan(c).any() or torch.isinf(c).any():
            print("Warning: Invalid values in final cost matrix")
